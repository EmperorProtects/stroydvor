...
from app.core.deps import get_current_admin, get_optional_user, get_current_user
...
async def my_orders(user: dict = Depends(get_current_user)):
    ...
    Ищет по user_id ИЛИ по номеру телефона (для заказов оформленных до регистрации).
    """
    db = get_database()
    query: dict = {"$or": [{"user_id": user["id"]}]}
    # Добавляем поиск по телефону если он есть
    if user.get("phone"):
        query["$or"].append({"customer_phone": user["phone"]})
    docs = await db.orders.find(query).sort("created_date", -1).to_list(200)
    # Попутно привязываем найденные заказы без user_id к пользователю
    orphan_ids = [d["_id"] for d in docs if not d.get("user_id")]
    if orphan_ids:
