...
import { ShoppingCart, Search, X, ChevronRight, Phone, MapPin, Grid3X3, Heart, Sun, Moon, User, Package } from "lucide-react";
...
export default function Header() {
  ...
  const menuItems = [
    ...
    { label: t("contacts"), desc: settings.city ? `${settings.city}, ${t("phonesDesc") || "телефоны отделов"}` : t("phonesDesc") || "г. Астана, телефоны отделов", link: "/contacts", icon: Phone },
  ...
  const IconBtn = ({ to, icon: Icon, label, badge }) => (
    <Link
      to={to}
      className="relative flex flex-col items-center gap-0.5 px-1 text-foreground hover:text-[#C0392B] transition-colors shrink-0"
    >
    ...
    <>
      ...
      >
        ...
        <div className="px-5 py-3 border-t border-border flex items-center gap-3">
          ...
          <button onClick={nextLang} className="text-sm font-bold text-muted-foreground hover:text-foreground uppercase transition-colors">
            {lang}
          </button>
        </div>
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2 sm:py-3 flex items-center gap-2 sm:gap-3 w-full">
            ...
            <form onSubmit={handleSearch} className="flex-1 flex min-w-0">
              <input
                ...
                className="flex-1 min-w-0 h-9 sm:h-10 px-3 sm:px-4 text-sm rounded-l-lg border border-r-0 border-border outline-none bg-background text-foreground placeholder:text-muted-foreground"
              />
              <button
                type="submit"
            ...
            <button
              onClick={nextLang}
              className="hidden sm:flex items-center justify-center w-9 h-9 rounded-lg hover:bg-secondary transition-colors shrink-0 text-xs font-bold uppercase text-foreground"
              title={t("search")}
            >
              {lang}
            </button>
            ...
            <div className="hidden sm:flex items-center gap-3">
              ...
              <IconBtn to="/my-orders" icon={User} label={t("cabinet")} badge={0} />
            </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 sm:hidden bg-card border-t border-border">
        <div className="flex items-stretch">
          {[
            ...
            { to: "/my-orders", icon: User,         labelKey: "cabinet" },
